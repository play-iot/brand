...
const source = "design/logo-v1.png"; // Source image(s). `string`, `buffer` or array of `string`
const logo = path.join("./pack", "logo");
const manifest = path.join("./pack", "manifest");
...
const configuration = {
  ...
  callback = function (error, response) {
    ...
    Promise.all([
      ...response.images.map(each => fs.writeFile(path.join(logo, each.name), each.contents)),
      ...response.files.map(each => fs.writeFile(path.join(manifest, each.name), each.contents)),
      fs.writeFile(path.join(manifest, 'head.html'), `<!DOCTYPE html><html>\n<head>\n${response.html.join("\n")}\n</head>\n</html>`)
    ]).catch(err => {
